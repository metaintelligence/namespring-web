...
import { dayOfYear, daysInYear, jdUtcToKst, kstToJdUtc, roundToNearestMinute, } from './Vsop87dFallbackTimeUtils.js';
import { norm180, toDegrees, toRadians, wrap360, } from './Vsop87dFallbackMathUtils.js';
import { compareBoundaryMoments, findFirstBoundaryByKey, findLastBoundaryByKey, momentKey, } from './JeolBoundarySearch.js';
...
function apparentLongitudeDeg(jdTt) {
...
function deltaTSeconds(year, month, day, hour, minute) {
...
function solveJdUtc(guessYear, guessMonth, guessDay, targetLongitude) {
...
function solveBoundary(year, term) {
...
function sortedBoundaries(years) {
    return years
...
export const Vsop87dFallback = {
    ...
    sajuMonthIndexAt(year, month, day, hour, minute) {
    ...
    nextBoundaryAfter(year, month, day, hour, minute) {
    ...
    previousBoundaryAtOrBefore(year, month, day, hour, minute) {
        const allBoundaries = sortedBoundaries([year - 1, year]);
        const key = momentKey(year, month, day, hour, minute);
...
//# sourceMappingURL=Vsop87dFallback.js.map