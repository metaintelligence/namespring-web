...
export function buildYongshinReasoning(yongshin, yongshinPriority) {
    ...
    const yongshinReasoning = [
        '【용신 결정 3단계 우선순위 체계】',
    ];
    if (hasTier1) {
        const tier1Rec = yongshin.recommendations.find(r => tier1Types.includes(r.type));
        yongshinReasoning.push(`▶ 제1단계 (격국 본질): ${YONGSHIN_TYPE_INFO[tier1Rec.type].koreanName} 적용됨 — ` +
    ...
    if (!hasTier1) {
        yongshinReasoning.push(`▶ 제3단계 (기본): 억부+조후 기본 결정, 우선순위=${yongshinPriority}`);
    }
    yongshinReasoning.push('');
    yongshinReasoning.push('【개별 방법론 상세】');
    for (const rec of yongshin.recommendations) {
        yongshinReasoning.push(`[${YONGSHIN_TYPE_INFO[rec.type].koreanName}] ${rec.reasoning} ` +
            `→ 용신: ${ohaengKr(rec.primaryElement)}` +
            (rec.secondaryElement ? `, 희신: ${ohaengKr(rec.secondaryElement)}` : '') +
            ` (신뢰도 ${Math.round(rec.confidence * 100)}%)`);
    }
    yongshinReasoning.push('');
    yongshinReasoning.push(`억부-조후 일치도: ${yongshin.agreement} → 최종 신뢰도 ${Math.round(yongshin.finalConfidence * 100)}%`);
    if (yongshin.gisin) {
            ...
            const quality = yongshinMatch && !gisinMatch
                ...
                : gisinMatch && !yongshinMatch
                    ...
                    : yongshinMatch && gisinMatch
                        ? '길흉혼재'
                        : '평(平)';
            daeunReasoning.push(`${dp.startAge}세 ${CHEONGAN_INFO[dp.pillar.cheongan].hangul}${JIJI_INFO[dp.pillar.jiji].hangul}` +
...
export function buildSaeunReasoning(saeun, pillars, yongshin) {
    const saeunReasoning = [
        '세운은 60갑자 순환에 따른 해당 연도의 천간·지지 기운입니다. ' +
            '기준점: 1984년=갑자(甲子)년, 60년 주기로 반복.',
    ];
    if (saeun.length > 0) {
        const first = saeun[0];
        saeunReasoning.push(`${first.year}년: ${CHEONGAN_INFO[first.pillar.cheongan].hangul}${JIJI_INFO[first.pillar.jiji].hangul}` +
        ...
        for (const sp of saeun.slice(0, 5)) {
            ...
            for (const pos of PILLAR_POSITION_VALUES) {
                ...
                if (spStemInfo.eumyang === natalStemInfo.eumyang &&
                    ...
                    interactions.push(`${pos}천간충(${spStemInfo.hangul}↔${natalStemInfo.hangul})`);
                }
                const branchDiff = Math.abs(jijiOrdinal(sp.pillar.jiji) - jijiOrdinal(natalPillar.jiji));
                if (branchDiff === 6) {
                    interactions.push(`${pos}지지충(${JIJI_INFO[sp.pillar.jiji].hangul}↔${JIJI_INFO[natalPillar.jiji].hangul})`);
                }
            }
            const yongshinMatch = stemOh === yongshinEl || branchOh === yongshinEl;
            const gisinMatch = gisinEl != null && (stemOh === gisinEl || branchOh === gisinEl);
            ...
            const interactionStr = interactions.length > 0
                ? ` | ${interactions.join(', ')}`
                : '';
            saeunReasoning.push(`${sp.year}년 ${CHEONGAN_INFO[sp.pillar.cheongan].hangul}${JIJI_INFO[sp.pillar.jiji].hangul}: ` +
                `${quality}${interactionStr}`);
        }
    }
    return saeunReasoning;
}
//# sourceMappingURL=LuckTraceReasoningBuilders.js.map