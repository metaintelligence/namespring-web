...
export function buildCheonganRelationsTraceStep(cheonganRelations) {
    return tracedStep(ANALYSIS_KEYS.CHEONGAN_RELATIONS, `천간 관계 ${cheonganRelations.length}건 감지 (합/충/극 등).`, [`hits=${cheonganRelations.length}`], cheonganRelations.length === 0
...
export function buildResolvedJijiRelationsTraceStep(resolvedJijiRelations, allowBanhap) {
    return tracedStep(ANALYSIS_KEYS.RESOLVED_JIJI, `지지 관계 ${resolvedJijiRelations.length}건 상호작용 해소 및 점수 산출. ` +
        `반합 ${allowBanhap ? '허용' : '제외'}.`, resolvedJijiRelations.length > 0
        ? resolvedJijiRelations.map(r => `${r.hit.note}: ${r.outcome}(${r.score?.finalScore ?? '?'}점)`)
        : ['지지 관계 없음'], resolvedJijiRelations.length === 0
...
export function buildScoredCheonganRelationsTraceStep(scoredCheonganRelations) {
    return tracedStep(ANALYSIS_KEYS.SCORED_CHEONGAN, `천간 관계 ${scoredCheonganRelations.length}건 점수 산출 (합화 평가 반영).`, scoredCheonganRelations.length > 0
        ? scoredCheonganRelations.map(s => `${s.hit.note}: ${s.score.finalScore}점`)
        : ['천간 관계 없음'], scoredCheonganRelations.length === 0
        ? ['천간 관계가 없으므로 점수 산출 대상이 없습니다.']
        : scoredCheonganRelations.map(s => `${s.hit.note}: ${s.score.rationale} → ${s.score.finalScore}점`));
}
//# sourceMappingURL=RelationTraceBuilders.js.map