...
export function buildPalaceTraceStep(palace) {
    return tracedStep(ANALYSIS_KEYS.PALACE, `궁성(宮星) 분석 완료.`, PILLAR_POSITION_VALUES
        .filter(pos => palace[pos]?.sipseong != null)
        .map(pos => `${pos}=${palace[pos].palaceInfo.koreanName}: ${SIPSEONG_INFO[palace[pos].sipseong].koreanName}`), PILLAR_POSITION_VALUES
...
//# sourceMappingURL=PalaceTraceBuilders.js.map