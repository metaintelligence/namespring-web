...
import { GYEOKGUK_YONGSHIN_TABLE, ILHAENG_TYPE_TO_OHAENG, } from './YongshinRuleCatalog.js';
import { categoryToOhaeng, countChartElements, ohaengKorean, } from './YongshinDecisionSupport.js';
const PRIMARY_TONGGWAN_CONFLICT_THRESHOLD = 3;
const SECONDARY_TONGGWAN_CONFLICT_THRESHOLD = 4;
function buildRecommendation(type, primaryElement, secondaryElement, confidence, reasoning) {
    return { type, primaryElement, secondaryElement, confidence, reasoning };
}
const TONGGWAN_ROLE_LABELS = {
    self: '비겁', controller: '관성', siksang: '식상', inseong: '인성', jaeseong: '재성',
};
const TONGGWAN_PATTERNS = [
...
function buildTonggwanReasoning(pattern, elements) {
...
export function eokbuYongshin(dayMasterOhaeng, isStrong) {
    ...
    return buildRecommendation(YongshinType.EOKBU, primary, secondary, 0.7, reasoning
        .replace('{dm}', `${dayMasterOhaeng}`)
        .replace('{primary}', `${primary}`)
        .replace('{secondary}', `${secondary}`));
}
export function johuYongshin(dayMaster, monthBranch) {
    const entry = JohuTable.lookup(dayMaster, monthBranch);
    const ci = CHEONGAN_INFO[dayMaster];
    const ji = JIJI_INFO[monthBranch];
    return buildRecommendation(YongshinType.JOHU, entry.primary, entry.secondary, 0.8, `조후(調候): ${ci.hangul}(${ci.hanja}) 일간 ` +
        `${ji.hangul}(${ji.hanja})월생, ` +
        `궁통보감 기준 ${entry.primary} 필요` +
        (entry.secondary != null ? `, 보조 ${entry.secondary}` : ''));
}
export function tongwanYongshin(pillars, dayMasterOhaeng, hapHwaEvaluations = []) {
    ...
    for (const pattern of TONGGWAN_PATTERNS) {
        ...
        return buildRecommendation(YongshinType.TONGGWAN, elements[pattern.mediator], null, pattern.confidence, buildTonggwanReasoning(pattern, elements));
    }
    return null;
}
const identityOhaeng = value => value;
function buildJeonwangModeRule(counterTransforms, counterTemplate, followTransforms, followTemplate) {
    return {
...
function resolveJeonwang(type, dayMasterOhaeng, mode) {
...
export function jeonwangYongshin(_pillars, dayMasterOhaeng, gyeokgukResult, config) {
    const mode = config.jonggyeokYongshinMode;
    const resolution = resolveJeonwang(gyeokgukResult.type, dayMasterOhaeng, mode);
    if (resolution == null)
        return null;
    return buildRecommendation(YongshinType.JEONWANG, resolution.primary, resolution.secondary, 0.75, `전왕(專旺): ${resolution.reasoning}`);
}
export function gyeokgukYongshin(dayMasterOhaeng, gyeokgukResult) {
    ...
    return buildRecommendation(YongshinType.GYEOKGUK, primary, secondary, 0.65, `격국용신(格局用神): ${typeInfo.koreanName} — ${rule.reasoning}`);
}
export function byeongyakYongshin(pillars, dayMasterOhaeng, isStrong, hapHwaEvaluations = []) {
    const elementCounts = countChartElements(pillars, hapHwaEvaluations);
    ...
    if (medicine === dayMasterOhaeng && !isStrong)
        return null;
    if (medicine === diseaseElement)
        return null;
    return buildRecommendation(YongshinType.BYEONGYAK, medicine, OhaengRelations.generates(medicine), 0.55, `병약(病藥): ${ohaengKorean(diseaseElement)}이(가) ${diseaseCount}개로 과다(병), ` +
        `${ohaengKorean(medicine)}이(가) 이를 제어(약). ` +
        `유병방귀(有病方貴), 무약불귀(無藥不貴).`);
}
export function hapwhaYongshin(gyeokgukResult, hapHwaEvaluations) {
    ...
    return buildRecommendation(YongshinType.HAPWHA_YONGSHIN, primary, secondary, 0.75, `합화용신(合化用神): ${typeInfo.koreanName}(${typeInfo.hanja}) — ` +
