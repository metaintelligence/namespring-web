...
function leapMonthOf(data) {
    return (data >>> 12) & 0x000F;
}
function totalLunarDays(data) {
    return (data >>> 17) & 0x01FF;
}
function leapMonthDays(data) {
    return ((data >>> 16) & 0x01) > 0 ? BIG_MONTH : SMALL_MONTH;
}
function monthDays(data, month) {
    return ((data >>> (12 - month)) & 0x01) > 0 ? BIG_MONTH : SMALL_MONTH;
}
function isSolarLeapYear(data) {
    return ((data >>> 30) & 0x01) > 0;
}
function lunarAbsDays(year, month, day, isIntercalation) {
    let days = 0;
...
function solarAbsDays(year, month, day) {
    let days = 0;
    for (let y = BASE_YEAR; y < year; y++) {
...
function isSolarInRange(year, month, day) {
...
export function lunarToSolar(lunarDate) {
    if (lunarDate.year < MIN_LUNAR_YEAR || lunarDate.year > MAX_LUNAR_YEAR)
        return null;
    if (!isValidLunarDate(lunarDate))
        return null;
    const absDays = lunarAbsDays(lunarDate.year, lunarDate.month, lunarDate.day, lunarDate.isLeapMonth);
...
export function solarToLunar(solarDate) {
    if (!isSolarInRange(solarDate.year, solarDate.month, solarDate.day))
        return null;
...
//# sourceMappingURL=KoreanLunarAlgorithmicConverter.js.map