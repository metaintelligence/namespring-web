...
export function branchKorean(j) {
    const NAMES = {
...
export function ohaengKorean(oh) {
    const NAMES = {
...
export function positionKorean(pos) {
    const NAMES = {
...
export function pillarAt(ps, pos) {
    switch (pos) {
...
export function strengthLevelKorean(level) {
    const NAMES = {
...
export function ohaengRelationNote(source, target) {
    if (source === target)
        return `${ohaengKorean(source)} 동일 → 비겁 부조`;
    if (OhaengRelations.generates(source) === target)
        return `${ohaengKorean(source)}→${ohaengKorean(target)} 상생`;
    if (OhaengRelations.controls(source) === target)
        return `${ohaengKorean(source)}→${ohaengKorean(target)} 상극`;
    if (OhaengRelations.generatedBy(source) === target)
        return `${ohaengKorean(target)}→${ohaengKorean(source)} 역생`;
    if (OhaengRelations.controlledBy(source) === target)
        return `${ohaengKorean(target)}→${ohaengKorean(source)} 역극`;
    return `${ohaengKorean(source)}↔${ohaengKorean(target)}`;
}
//# sourceMappingURL=NarrativeFormatting.js.map