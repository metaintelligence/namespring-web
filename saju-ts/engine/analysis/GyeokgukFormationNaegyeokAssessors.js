...
import { hasHiddenGyeobJae, hasHiddenPyeonIn, hasHiddenSangGwan, hasSikSinStrong, } from './GyeokgukFormationProfile.js';
import { buildNotAssessedFormation, evaluateFormationRule, } from './GyeokgukFormationShared.js';
function createJaeCommonSpec(patternName) {
    return {
        patternName,
...
function createInCommonSpec(patternName) {
    return {
        patternName,
        ...
        support: (p) => (p.hasGwan && p.hasInseong) ||
            (p.hasPyeonGwan && !p.isStrong) ||
            (p.isStrong && p.hasSiksang),
    };
}
const NAEGYEOK_RULES = {
    [GyeokgukType.JEONGGWAN]: {
        ...
        support: (p) => (p.hasJae && !p.hasSangGwan) ||
    ...
    [GyeokgukType.PYEONIN]: {
        ...
        support: (p) => (p.hasPyeonGwan && p.hasInseong) ||
            (p.hasJae && p.isStrong),
    },
    [GyeokgukType.GEONROK]: {
        ...
        support: (p) => (p.hasJeongGwan && (p.hasJae || p.hasInseong)) ||
