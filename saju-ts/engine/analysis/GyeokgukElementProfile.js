...
export function effectiveOhaeng(position, stem, hapHwaEvaluations) {
    ...
    if (activeHap?.state === HapState.HAPWHA)
        return activeHap.resultOhaeng;
    return CHEONGAN_INFO[stem].ohaeng;
}
export function buildElementProfile(dayMaster, pillars, hapHwaEvaluations) {
    let bigyeop = 0;
    let siksang = 0;
    let jae = 0;
    let gwan = 0;
    ...
    function count(category) {
        switch (category) {
            case SipseongCategory.BIGYEOP:
                bigyeop++;
                break;
            case SipseongCategory.SIKSANG:
                siksang++;
                break;
            case SipseongCategory.JAE:
                jae++;
                break;
            case SipseongCategory.GWAN:
                gwan++;
                break;
            case SipseongCategory.INSEONG:
                inseong++;
                break;
        }
    }
...
export { ohaengKorean };
//# sourceMappingURL=GyeokgukElementProfile.js.map