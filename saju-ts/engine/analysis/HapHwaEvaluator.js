...
import { buildReasoning, checkOpposition, computePresenceBonus, determineStateAndConfidence, positionOrdinal, } from './HapHwaEvaluatorHelpers.js';
export const HapHwaEvaluator = {
    evaluate(pillars, strictness = HapHwaStrictness.STRICT_FIVE_CONDITIONS, dayMasterNeverHapGeo = true) {
        ...
        for (let i = 0; i < positions.length; i++) {
            for (let j = i + 1; j < positions.length; j++) {
                ...
                if (!HAP_TABLE.has(key))
                    continue;
                const posKey = [pos1, pos2].sort().join('-');
                if (seen.has(posKey))
                    continue;
                seen.add(posKey);
                const evaluation = evaluateHapHwaPair(stem1, pos1, stem2, pos2, pillars.month.jiji, pillars, strictness, dayMasterNeverHapGeo);
                if (evaluation !== null) {
    ...
    evaluatePair(stem1, pos1, stem2, pos2, monthBranch, pillars, strictness = HapHwaStrictness.STRICT_FIVE_CONDITIONS, dayMasterNeverHapGeo = true) {
        const entry = HAP_TABLE.get(stemPairKey(stem1, stem2));
        if (entry === undefined)
            return null;
        const { resultOhaeng, hapName } = entry;
        const dayMaster = pillars.day.cheongan;
        const dayMasterInvolved = (stem1 === dayMaster && pos1 === PillarPosition.DAY) ||
        ...
        const { state, confidence } = determineStateAndConfidence(adjacent, seasonSupport, presenceBonus, hasOpposition, strictness);
        const reasoning = buildReasoning(hapName, stem1, pos1, stem2, pos2, resultOhaeng, monthBranch, state, adjacent, seasonSupport, presenceBonus, hasOpposition);
        return {
            stem1,
            stem2,
            position1: pos1,
    ...
    isSeasonSupporting(monthBranch, targetElement) {
...
//# sourceMappingURL=HapHwaEvaluator.js.map