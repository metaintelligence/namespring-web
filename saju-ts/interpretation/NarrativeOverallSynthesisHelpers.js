...
import { GyeokgukCategory, GyeokgukType, GYEOKGUK_TYPE_INFO, } from '../domain/Gyeokguk.js';
...
import { branchKorean, formatPillar, ohaengKorean, positionKorean, stemKorean, strengthLevelKorean, } from './NarrativeFormatting.js';
...
export function buildCausalChain(a) {
    const steps = [];
    ...
    if (a.yongshinResult && a.daeunInfo) {
        ...
        const favorableDaeun = a.daeunInfo.daeunPillars.filter(dp => CHEONGAN_INFO[dp.pillar.cheongan].ohaeng === yongshinOh ||
        ...
        if (yr.gisin != null) {
            ...
            const unfavorableDaeun = a.daeunInfo.daeunPillars.filter(dp => CHEONGAN_INFO[dp.pillar.cheongan].ohaeng === gisinOh ||
                JIJI_INFO[dp.pillar.jiji].ohaeng === gisinOh);
            if (unfavorableDaeun.length > 0) {
                const periods = unfavorableDaeun.slice(0, 3).map(dp => `${dp.startAge}~${dp.startAge + 9}세(${formatPillar(dp.pillar)})`).join(', ');
...
export function buildStrengthToGyeokgukTransition(sr, gr) {
    if (!sr)
        return '';
...
export function buildMultiMethodYongshinBasis(yr, sr) {
    if (yr.recommendations.length === 0)
        return '';
    const parts = [];
    for (const rec of yr.recommendations) {
        ...
        parts.push(rec.type === YongshinType.JOHU ? `조후론: ${compactText(rec.reasoning)}`
...
export function daeunBriefAssessment(sipseong, unseong) {
    let energyTier;
    if (unseong === SibiUnseong.JE_WANG || unseong === SibiUnseong.GEON_ROK)
        energyTier = '최고조';
    else if (unseong === SibiUnseong.GWAN_DAE || unseong === SibiUnseong.JANG_SAENG)
        energyTier = '상승';
    else if ([SibiUnseong.MOK_YOK, SibiUnseong.SWOE, SibiUnseong.TAE, SibiUnseong.YANG].includes(unseong))
        energyTier = '보통';
    else
        energyTier = '침체';
    const SIPSEONG_TONES = {
...
export function buildLifePathSynthesis(a) {
    const parts = [];
    if (a.gyeokgukResult) {
        const gr = a.gyeokgukResult;
        const gti = GYEOKGUK_TYPE_INFO[gr.type];
...
//# sourceMappingURL=NarrativeOverallSynthesisHelpers.js.map