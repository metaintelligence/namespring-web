...
import { STRENGTH_LEVEL_INFO } from '../../domain/StrengthResult.js';
import { tracedStep } from './TraceHelpers.js';
export function buildGongmangTraceStep(gongmang, pillars) {
    const gvb = gongmang.voidBranches;
    return tracedStep(ANALYSIS_KEYS.GONGMANG, `공망(空亡) 계산 — ${JIJI_INFO[gvb[0]].hangul}(${JIJI_INFO[gvb[0]].hanja})·` +
...
export function buildStrengthTraceStep(strength, dmInfo, strengthThreshold) {
    return tracedStep(ANALYSIS_KEYS.STRENGTH, `일간 ${dmInfo.hangul}(${dmInfo.hanja}) 강약 판정 — ` +
...
//# sourceMappingURL=GongmangStrengthTraceBuilders.js.map