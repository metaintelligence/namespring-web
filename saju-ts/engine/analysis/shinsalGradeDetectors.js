import { ShinsalGrade, ShinsalType } from '../../domain/Shinsal.js';
import { MUNCHANG_TABLE, YANGIN_TABLE, HAKDANG_TABLE, GEUMYEO_TABLE, BAEKHO_TABLE, HONGYEOM_TABLE, AMNOK_TABLE, CHEONGWAN_TABLE, MUNGOK_TABLE, GUGIN_TABLE, CHEONBOK_GWIIN_TABLE, HYEOLINSAL_TABLE, CHEONJU_GWIIN_TABLE, CHEONDEOK_TABLE, WOLDEOK_TABLE, CHEONDEOK_HAP_TABLE, WOLDEOK_HAP_TABLE, } from './ShinsalCatalog.js';
import { detectByStemTable, detectSamhapShinsal, detectByMonthMixed, detectByYearBranchOffset, } from './shinsalHelpers.js';
import { detectCheonulGwiin, detectTaegukGwiin, detectWonjin, detectGoegang, detectGosin, detectGwasuk, detectBokseong, detectCheonui, detectCheollaJimang, detectGyeokgak, detectYukhaesal, detectGoransal, } from './shinsalSpecificDetectors.js';
function detectorFromSpec(spec, gwiiinVariant, refBranch) {
    switch (spec.kind) {
        case 'cheonulGwiin':
...
export function detectorsForGrade(grade, gwiiinVariant, refBranch) {
    return GRADE_SPECS[grade].map((spec) => detectorFromSpec(spec, gwiiinVariant, refBranch));
}
export function detectorsForScope(grade, gwiiinVariant, refBranch) {
    if (grade !== null)
        return detectorsForGrade(grade, gwiiinVariant, refBranch);
    return ALL_GRADES.flatMap((gradeValue) => detectorsForGrade(gradeValue, gwiiinVariant, refBranch));
}
//# sourceMappingURL=shinsalGradeDetectors.js.map